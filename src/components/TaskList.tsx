    ...
    <div className="p-5">
      ...
      <form onSubmit={handleAdd} className="flex gap-2 mb-3">
        <input
          ...
          className="flex-1 px-3 py-2 rounded-[var(--radius-md)] border border-border bg-surface text-[15px] text-text-primary placeholder:text-text-4 focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all"
        />
        <button
          type="submit"
            ...
            <li key={task.id} className="relative flex items-center gap-3 pl-5 pr-10 py-[9px] border-t border-border hover:bg-surface-hover transition-[background] duration-[120ms]">
              ...
              {editingId === task.id ? (
                <input
                  ...
                  className="flex-1 min-w-0 text-[15px] text-text-primary bg-transparent border-none py-0"
                />
              ) : (
                <span
                  onClick={() => startEditing(task)}
                  className={`text-[15px] truncate cursor-text flex-1 min-w-0 ${task.completed ? "text-text-3 line-through" : "text-text-primary"}`}
                >
                  {task.title}
                </span>
              )}
